	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			if (user == null) {
				...
				//valid login --1. Create a Cookie to store user validated user details
...
//				pw.print("<h5> User Details "+user+"</h5>");
				if (user.getRole().equals("voter")) {
					...
					/* 
					 * WC - 1. clears/ empties te PW's buffer
					 * 2. Sends temp redirect resp - SC 302, location="candidate_list" 
					 * 2.Set-Cookie - cookie name -value
					 * resp body -empty
					 * 3.Browser-checks privacy settings
					 * cookies blocker -- cookies won't be stored -- can't remember
					 * acceoted --cookie age --def valuee-1=>
					 * saves it in cache(cookie storage)
					 * 4. Clnt browser - NEXT req (redirect)
